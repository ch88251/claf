...
/**
 * Icon factory for Industrial Look and Feel.
 * Currently delegates to BaseIcons - can be customized for industrial-specific icons.
 */
public class IndustrialIconFactory implements AbstractIconFactory {
    
    private static IndustrialIconFactory instance = null;
    
    public static IndustrialIconFactory getInstance() {
        if (instance == null) {
            instance = new IndustrialIconFactory();
        }
        return instance;
    }

    // Delegate all icon methods to BaseIcons for now
    // These can be overridden to provide industrial-specific icons
