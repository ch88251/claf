    ...
    public void paint(Graphics g) {
        ...
        if (UIManager.getLookAndFeel() instanceof AbstractLookAndFeel) {
            ...
            if (orientation == JSplitPane.HORIZONTAL_SPLIT) {
                CLAFUtilities.fillVerGradient(g, colors, 0, 0, width, height);
            ...
            public void paint(Graphics g) {
                ...
                if (getModel().isArmed()) {
                    if (getModel().isPressed()) {
                        CLAFUtilities.draw3DBorder(g, ColorHelper.darker(color, 30), ColorHelper.brighter(color, 80), 0, 0, w, h);
                    } else {
                        CLAFUtilities.draw3DBorder(g, ColorHelper.brighter(color, 80), ColorHelper.darker(color, 30), 0, 0, w, h);
                    }
                }
            }
