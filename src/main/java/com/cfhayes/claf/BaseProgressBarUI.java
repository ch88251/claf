...
public class BaseProgressBarUI extends BasicProgressBarUI {
    ...
    protected void paintIndeterminate(Graphics g, JComponent c) {
        ...
        if (boxRect != null) {
            ...
            CLAFUtilities.draw3DBorder(g, cHi, cLo, boxRect.x + 1, boxRect.y + 1, boxRect.width - 2, boxRect.height - 2);
            CLAFUtilities.fillHorGradient(g, colors, boxRect.x + 2, boxRect.y + 2, boxRect.width - 4, boxRect.height - 4);
        }
    ...
    protected void paintDeterminate(Graphics g, JComponent c) {
        ...
        Color cLo = ColorHelper.darker(colors[colors.length - 1], 10);
        if (progressBar.getOrientation() == JProgressBar.HORIZONTAL) {
            if (CLAFUtilities.isLeftToRight(progressBar)) {
                CLAFUtilities.draw3DBorder(g, cHi, cLo, 2, 2, amountFull - 2, h - 2);
                CLAFUtilities.fillHorGradient(g, colors, 3, 3, amountFull - 4, h - 4);
            } else {
                CLAFUtilities.draw3DBorder(g, cHi, cLo, w - amountFull + 2, 2, w - 2, h - 2);
                CLAFUtilities.fillHorGradient(g, colors, w - amountFull + 3, 3, w - 4, h - 4);
            }
        } else { // VERTICAL
            CLAFUtilities.draw3DBorder(g, cHi, cLo, 2, 2, w - 2, amountFull - 2);
            CLAFUtilities.fillVerGradient(g, colors, 3, 3, w - 4, amountFull - 4);
        }
