...
/**
 * Icon factory for Financial Look and Feel.
 * Currently delegates to BaseIcons - can be customized for financial-specific icons.
 */
public class FinancialIconFactory implements AbstractIconFactory {
    
    private static FinancialIconFactory instance = null;
    
    public static FinancialIconFactory getInstance() {
        if (instance == null) {
            instance = new FinancialIconFactory();
        }
        return instance;
    }

    // Delegate all icon methods to BaseIcons for now
    // These can be overridden to provide financial-specific icons (dollar signs, charts, etc.)
