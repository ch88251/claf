...
public class FinancialButtonUI extends BaseButtonUI {

    public static ComponentUI createUI(JComponent c) {
        return new FinancialButtonUI();
    }
    ...
    protected void paintBackground(Graphics g, AbstractButton b) {
        ...
        if (((width < 64) || (height < 16)) && ((b.getText() == null) || b.getText().equals(""))) {
            ...
            if (b.getBackground() instanceof ColorUIResource) {
                if (!model.isEnabled()) {
                    colors = FinancialLookAndFeel.getTheme().getDisabledColors();
                } else if (model.isRollover()) {
                    colors = FinancialLookAndFeel.getTheme().getRolloverColors();
                } else {
                    colors = FinancialLookAndFeel.getTheme().getButtonColors();
                }
            } else {
            ...
            if (model.isEnabled()) {
                g2D.setColor(FinancialLookAndFeel.getFrameColor());
            } else {
                g2D.setColor(ColorHelper.brighter(FinancialLookAndFeel.getFrameColor(), 20));
            }
            ...
            int d = height - 2;
            Color color = FinancialLookAndFeel.getTheme().getSelectionBackgroundColor();
            g2D.setColor(color);
