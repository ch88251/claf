...
public class BaseBorders {
    ...
    private static Border menuBarBorder = null;
    ...
    public static class PopupMenuBorder extends AbstractBorder implements UIResource {
        ...
        public void paintLogo(Graphics2D g2D, int w, int h) {
            ...
            imageGraphics.drawString(CLAFUtilities.getClippedText(AbstractLookAndFeel.getTheme().getLogoString(), fm, h - 16), xs - 1, ys + 1);

            imageGraphics.setColor(Color.white);
            imageGraphics.drawString(CLAFUtilities.getClippedText(AbstractLookAndFeel.getTheme().getLogoString(), fm, h - 16), xs, ys);
    ...
    public static class ToolButtonBorder implements Border, UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            ...
            CLAFUtilities.draw3DBorder(g, frameHiColor, frameLoColor, x, y, w, h);
            if ((model.isPressed() && model.isArmed()) || model.isSelected()) {
                CLAFUtilities.draw3DBorder(g, frameLoColor, frameHiColor, x, y, w, h);
            } else {
                CLAFUtilities.draw3DBorder(g, frameLoColor, frameHiColor, x, y, w, h);
                CLAFUtilities.draw3DBorder(g, frameHiColor, frameLoColor, x + 1, y + 1, w - 2, h - 2);
            }
        }
    ...
    public static class PaletteBorder extends AbstractBorder implements UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            if (CLAFUtilities.isFrameActive((JComponent) c)) {
        ...
        public boolean isActive(Component c) {
            ...
            if (c instanceof JDialog) {
                ...
                if (dlg.getParent() instanceof JComponent) {
                    return CLAFUtilities.isActive((JComponent) (dlg.getParent()));
                }
            } else if (c instanceof JInternalFrame) {
                JInternalFrame frame = (JInternalFrame) c;
                active = frame.isSelected();
                if (active) {
                    return CLAFUtilities.isActive(frame);
                }
            } else if (c instanceof JRootPane) {
                JRootPane jp = (JRootPane) c;
                if (jp.getTopLevelAncestor() instanceof Window) {
                    Window window = (Window) jp.getTopLevelAncestor();
                    return CLAFUtilities.isWindowActive(window);
                }
            }
            return active;
        }
    ...
    public static class Down3DBorder extends AbstractBorder implements UIResource {
        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            ...
            CLAFUtilities.draw3DBorder(g, ColorHelper.darker(frameColor, 20), ColorHelper.brighter(frameColor, 80), x, y, w, h);
        }
