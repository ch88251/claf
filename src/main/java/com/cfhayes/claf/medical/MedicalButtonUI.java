...
public class MedicalButtonUI extends BaseButtonUI {

    public static ComponentUI createUI(JComponent c) {
        return new MedicalButtonUI();
    }
    ...
    protected void paintBackground(Graphics g, AbstractButton b) {
        ...
        if (((width < 64) || (height < 16)) && ((b.getText() == null) || b.getText().equals(""))) {
            ...
            if (b.getBackground() instanceof ColorUIResource) {
                if (!model.isEnabled()) {
                    colors = MedicalLookAndFeel.getTheme().getDisabledColors();
                } else if (model.isRollover()) {
                    colors = MedicalLookAndFeel.getTheme().getRolloverColors();
                } else {
                    colors = MedicalLookAndFeel.getTheme().getButtonColors();
                }
            } else {
            ...
            if (model.isEnabled()) {
                g2D.setColor(MedicalLookAndFeel.getFrameColor());
            } else {
                g2D.setColor(ColorHelper.brighter(MedicalLookAndFeel.getFrameColor(), 20));
            }
            ...
            int d = height - 2;
            Color color = MedicalLookAndFeel.getTheme().getSelectionBackgroundColor();
            g2D.setColor(color);
