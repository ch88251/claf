...
// Private instance data
    ...
    private void installTabContainer() {
        if (CLAFUtilities.getJavaVersion() >= 1.6) {
    ...
    protected void paintText(Graphics g, int tabPlacement, Font font, FontMetrics metrics, int tabIndex, String title, Rectangle textRect, boolean isSelected) {
        ...
        if (v != null) {
            ...
            if (tabPane.isEnabled() && tabPane.isEnabledAt(tabIndex)) {
                ...
                CLAFUtilities.drawStringUnderlineCharAt(tabPane, g, title, mnemIndex, textRect.x, textRect.y + metrics.getAscent());

            } else { // tab disabled
                g.setColor(tabPane.getBackgroundAt(tabIndex).brighter());
                CLAFUtilities.drawStringUnderlineCharAt(tabPane, g, title, mnemIndex, textRect.x, textRect.y + metrics.getAscent());
                g.setColor(tabPane.getBackgroundAt(tabIndex).darker());
                CLAFUtilities.drawStringUnderlineCharAt(tabPane, g, title, mnemIndex, textRect.x - 1, textRect.y + metrics.getAscent() - 1);
            }
        }
    }
    ...
    protected void paintTopTabBorder(int tabIndex, Graphics g, int x1, int y1, int x2, int y2, boolean isSelected) {
        ...
        boolean leftToRight = CLAFUtilities.isLeftToRight(tabPane);
    ...
    protected void paintTabBackground(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h, boolean isSelected) {
        ...
        switch (tabPlacement) {
            ...
            default:
                if (isSelected) {
                    CLAFUtilities.fillHorGradient(g, colorArr, x + 1, y + 1, w - 1, h + 1);
                } else {
                    CLAFUtilities.fillHorGradient(g, colorArr, x + 1, y + 1, w - 1, h - 1);
                }
                break;
            case LEFT:
                if (isSelected) {
                    CLAFUtilities.fillHorGradient(g, colorArr, x + 1, y + 1, w, h - 1);
                } else {
                    CLAFUtilities.fillHorGradient(g, colorArr, x + 1, y + 1, w - 1, h - 1);
                }
                break;
            case BOTTOM:
                if (isSelected) {
                    CLAFUtilities.fillHorGradient(g, colorArr, x + 1, y - 1, w - 1, h);
                } else {
                    CLAFUtilities.fillHorGradient(g, colorArr, x + 1, y, w - 1, h - 1);
                }
                break;
            case RIGHT:
                if (isSelected) {
                    CLAFUtilities.fillHorGradient(g, colorArr, x, y + 1, w, h - 1);
                } else {
                    CLAFUtilities.fillHorGradient(g, colorArr, x + 1, y + 1, w - 1, h - 1);
                }
                break;
